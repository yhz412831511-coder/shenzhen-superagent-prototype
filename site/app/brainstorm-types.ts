...
export type BrainstormPolicy = {
  ...
  sourceId: string;
  sourceDate: string;
  publicationStatus: 'published_verified' | 'published_pending_check' | 'unpublished_or_unknown';
  topicIds: string[];
};
...
export type BrainstormConflict = {
  ...
  category: 'direction' | 'boundary' | 'responsibility' | 'policy' | 'commitment' | 'wording';
  participantIds: string[];
...
export type BrainstormAction =
  | { type: 'issue-update'; taskId: string; field: 'goal' | 'audience' | 'period' | 'quality'; value: string }
  ...
  | { type: 'scope-toggle'; taskId: string; field: 'crossOfficeOriginals' | 'personalMemory' }
  | { type: 'scope-confirm'; taskId: string }
  | { type: 'collect'; taskId: string }
  | { type: 'inquire'; taskId: string }
  | { type: 'decision-resolve'; taskId: string; conflictId: string; optionId: string }
  | { type: 'outline-confirm'; taskId: string }
  | { type: 'artifacts-generate'; taskId: string }
  | { type: 'fact-gap-resolve'; taskId: string; resolution: 'keep_limited' | 'remove_from_draft' }
  | { type: 'working-version-confirm'; taskId: string }
  | { type: 'pause'; taskId: string }
  | { type: 'resume'; taskId: string };

export type BrainstormEffect =
  | { type: 'message'; role: 'user' | 'assistant' | 'system'; text: string; anchor?: keyof BrainstormFlow['anchors'] }
  | { type: 'artifact'; name: string; body: string; source: string; evidenceIds: string[] };
