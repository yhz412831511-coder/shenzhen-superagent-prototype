...
export function brainstormReducer(
  ...
  if (action.type === 'issue-confirm') {
    ...
    effects.push(
      { type: 'message', role: 'user', text: '确认议题卡，请推荐参与岗位。' },
      {
        type: 'message',
        role: 'assistant',
        text: '议题已确认。我按五个议题组推荐13个岗位参与；将以已确认的汇报对象、时间范围和质量重点作为本次判断尺度。你可以调整，移除时会显示可能缺失的职责视角。',
        anchor: 'participants',
      },
    );
    return { state, effects };
  ...
  if (action.type === 'scope-confirm') {
    ...
    effects.push(
      ...
      },
      {
        type: 'message',
        role: 'assistant',
        text: '调用范围已确认。已有授权摘要不再重复索取；下一步先汇集13个岗位的独立输出，并按“目标尺度、既有决定、已核验变化、尚未解决问题”形成年度报告七个主题盘点。系统只会推荐优先处理顺序，不会直接发起讨论。',
        anchor: 'synthesis',
      },
    );
    return { state, effects };
  ...
  if (action.type === 'collect') {
    ...
    effects.push({
      type: 'message',
      role: 'assistant',
      text: '13个岗位已完成独立贡献，V0初步汇集版和七主题盘点已经形成。已按本次汇报目标比对既有主线：可确认的变化先保留，未解决的事实、责任和顺序问题另列补证清单。人工智能＋政务涉及8个岗位、3项实质分歧，并影响2027年任务排序和安全边界，建议优先处理；请由你确认是否先进入该主题。',
      anchor: 'synthesis',
    });
    return { state, effects };
  }
  if (action.type === 'inquire') {
    ...
    if (flow.phaseStatus === 'partial_ready') {
      ...
      effects.push(
        ...
        },
        {
          type: 'message',
          role: 'assistant',
          text: '第一轮岗位互证已完成：不以场景数量为首要成绩、稳定数据和可评测结果是试点条件、共性能力统一沉淀。与V0主线相比，新增了准入条件并收紧了对外承诺；仍有三项事实、责任和顺序问题列入补证清单，需要用具体场景进入第二轮核对。该主题处理后仍会回到其余六个主题和完整年度报告大纲。',
          anchor: 'round',
        },
      );
      return { state, effects };
    ...
    effects.push({
      type: 'message',
      role: 'assistant',
      text: '第二轮已用政务服务办理辅助和民生诉求趋势研判完成补证，数据、安全、评测、人工复核、日志和停用条件已经明确；未有来源支撑的成效和进展仍保留待核。场景牵引还是底座优先仍会改变年度任务排序，需要你决定，并保留选择理由。',
      anchor: 'decision',
    });
    return { state, effects };
  }
  if (action.type === 'decision-resolve') {
    ...
    effects.push(
      { type: 'message', role: 'user', text: `选择“${option.label}”。` },
      {
        type: 'message',
        role: 'assistant',
        text: `已记录决定：${option.impact} 选择理由是其将直接影响年度任务排序与资源安排。我已据此调整开篇、主题排序、2027年任务和结尾表述，形成综合大纲；仍待补证的事项不写入成效结论。`,
        anchor: 'outline',
      },
    );
    return { state, effects };
