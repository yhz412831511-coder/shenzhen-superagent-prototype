...
const start = () => {
  ...
  const created = createBrainstormFlow(taskId, '请协同形成2026年度工作报告', at);
  state.flows[taskId] = created.flow;
  return { state, effects: created.effects };
};

const act = (state, action) => brainstormReducer(state, { taskId, ...action }, at);
...
test('脑暴协作完成六阶段闭环并生成四项一致成果', () => {
  ...
  assert.equal(transition.state.flows[taskId].topics.filter((item) => item.status === 'ready').length, 7);
  transition = act(transition.state, { type: 'inquire' });
  assert.equal(transition.state.flows[taskId].conflicts.find((item) => item.kind === 'ordinary').status, 'resolved');
  assert.equal(transition.state.flows[taskId].phaseStatus, 'awaiting_high_impact_decision');
  ...
  const artifacts = transition.effects.filter((item) => item.type === 'artifact');
  ...
  transition = act(transition.state, { type: 'fact-gap-resolve', resolution: 'keep_limited' });
  ...
  assert.ok(transition.state.flows[taskId].artifactMeta.every((item) => item.verificationStatus === 'reviewed'));
});

test('关键职责和证据边界未满足时阻断推进', () => {
  let transition = start();
  transition = act(transition.state, { type: 'issue-confirm' });
  transition = act(transition.state, { type: 'participants-toggle', participantId: 'office' });
  const participantBlocked = act(transition.state, { type: 'participants-confirm' });
  assert.match(participantBlocked.notice, /必须保留办公室/);
  assert.equal(participantBlocked.state.flows[taskId].phaseStatus, 'awaiting_participants');
...
test('暂停后禁止业务推进，恢复后从原状态继续', () => {
  ...
  transition = act(transition.state, { type: 'issue-confirm' });
  assert.equal(transition.state.flows[taskId].phaseStatus, 'awaiting_participants');
});
